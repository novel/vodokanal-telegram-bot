...
class VodokanalParser(HTMLParser):
    HREF_REGEXP = "/news/operativnyy-monitoring/(?P<id>[0-9]+)/"
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if dict_attrs.get('class') == 'main_btn':
            ...
            if match:
                ...
                if match_id:
                    self._news_links[int(match_id)] = href
...
if __name__ == "__main__":
    ...
    c.execute(
        ...
        (news_id integer PRIMARY KEY, title text, date date,
    ...
    for news_id, news_url in sorted(parser._news_links.items()):
        ...
        msg_tokens.append(URL_BASE + news_url)
        message = u'\n'.join(msg_tokens)
